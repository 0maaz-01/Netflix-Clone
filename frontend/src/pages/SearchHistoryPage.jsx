...
const SearchHistoryPage = () => {
	...
	useEffect(() => {
		const getSearchHistory = async () => {
			try {
				const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/v1/search/history`);
	...
	const handleDelete = async (entry) => {
		try {
			await axios.delete(`${import.meta.env.VITE_API_URL}/api/v1/search/history/${entry.id}`);
